...
function manualPay($db, $usname, $usid, $purse, $sum, $sonfig_site){
	...
	if(isset($_POST["purse2"])){
		...
		if(($purse[0] == "P") && (strlen($purse) == 9 || strlen($purse) == 8)){
				...
				if($sum >= $minPay){
					...
					if($db->NumRows() == 0){
						if($sum <= $maxPay){
							...
							$_SESSION["pay_error"] = "<center><b><font color ='red'>Максимальная сумма для выплаты составляет {$maxPay} монет! Ваша заявка принята и будет обработана в ручном режиме!</font></b></center><BR />";
						}
					}else{
						if($gone_time < 86400){
							manualPay($db, $usname, $usid, $purse, $sum, $sonfig_site);
			...
			$_SESSION["pay_error"] = "<center><b><font color ='red'>Кошелек Payeer указан неверно! Смотрите образец!</font></b></center><BR />";
		}
