		...
		if($arBalance["auth_error"] == 0)
		{
			...
			if( ($balance) >= ($sum_pay)){
				...
				if (!empty($arTransfer["historyId"]))
				{
					...
					$_SESSION["pay_error"] = "<center><font color = 'green'><b>Выплачено!</b></font></center><BR />";

				}
				else
				{
					$_SESSION["pay_error"] = "<center><font color = 'red'><b>Внутреняя ошибка - сообщите о ней администратору!</b></font></center><BR />";
				}
			...
			$_SESSION["pay_error"] = "<center><font color = 'red'><b>Не удалось выплатить! Попробуйте позже</b></font></center><BR />";
		}
		  ...
		  if(($purse[0] == "P") && (strlen($purse) == 9 || strlen($purse) == 8)){
			...
			if($db->NumRows() == 0){
				...
				if($sum >= $minPay){
					...
					$db->Query("SELECT * FROM db_payment WHERE user_id = '$usid' AND status = '3' ORDER BY id DESC LIMIT 1");
					$last_pay = $db->FetchArray();
					$date = new DateTime();
					...
					if($db->NumRows() == 0){
						if($sum <= $maxPay){
							...
							$_SESSION["pay_error"] = "<center><b><font color ='red'>Максимальная сумма для автоматическо выплаты составляет {$maxPay} монет! Ваша заявка принята и будет обработана в ручном режиме!</font></b></center><BR />";
						}
					}else{
						if($gone_time < 86400){
							manualPay($db, $usname, $usid, $purse, $sum, $sonfig_site);
							$_SESSION["pay_error"] = "<center><b><font color = 'red'>Автоматическая выплата доступна раз в сутки! Ваша заявка принята и будет обработана в ручном режиме!</font></b></center><BR />";
						}else{
							if($sum <= $maxPay){
								echo autoPay($config, $sum, $sonfig_site, $purse, $db, $usname, $usid);
							}else{
								manualPay($db, $usname, $usid, $purse, $sum, $sonfig_site);
								$_SESSION["pay_error"] = "<center><b><font color ='red'>Максимальная сумма для автовыплаты составляет {$maxPay} монет! Ваша заявка принята и будет обработана в ручном режиме!</font></b></center><BR />";
							}
						}
					}

				}else {
					$_SESSION["pay_error"] = "<center><b><font color = 'red'>Минимальная сумма для выплаты составляет {$minPay} монет!</font></b></center><BR />";
				}
			}else{
				$_SESSION["pay_error"] = "<center><font color = 'red'><b>У Вас есть необработанная заявка, дождитесь выплаты!</b></font></center><BR />";
			}

		}else{
			$_SESSION["pay_error"] = "<center><b><font color ='red'>Кошелек Payeer указан неверно!</font></b></center><BR />";
		}
...
<input id="sum" class="input_text w340" type="number" autocomplete="off" max="<?=round($user_data["money_p"]); ?>" min="<?= $minPay ?>" value="<?=round($user_data["money_p"]); ?>" name="sum" onkeyup="PaymentSum();" />
</div>
...
<input class="subm_button" type="submit" name="swap" value="Заказать выплату"/>
	...
	<div class="webmoney_right">
		...
		Оплата оказываемых в интернете услуг, перевод денег по всему миру (как внутри самой системы, так и на внешние счета), 
