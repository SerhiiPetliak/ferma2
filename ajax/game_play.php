...
$db->Query("INSERT INTO `db_knb_games_list` (sum, user, user2, date, status) VALUES ('$sum', '$uname', '$login', '".time()."', '�����')");			
			$db->Query("DELETE FROM `db_games_knb` WHERE `id` = ".intval($_POST["id"]));
			
			echo "<script type='text/javascript'>$('.play-".intval($_POST["id"])."').html('�����');</script>";
		...
		$login = $row["login"];
			...
			$db->Query("DELETE FROM `db_games_knb` WHERE `id` = ".intval($_POST["id"]));
			echo "<script type='text/javascript'>$('.play-".intval($_POST["id"])."').html('<font color=\"#f00\">���������</font>');
</script>";
		}else{
				$sum = $row["summa"];
		$login = $row["login"];		
			$db->Query("UPDATE `db_users_b` SET `money_b` = `money_b` + ".round($row["summa"]*(1-playCom),2)." WHERE `user`  = '".$uname."'");
			$db->Query("INSERT INTO `db_knb_games_list` (sum, user, user2, date, status) VALUES ('$sum', '$uname', '$login', '".time()."', '������� y ������')");
			$db->Query("DELETE FROM `db_games_knb` WHERE `id` = ".intval($_POST["id"]));
			echo "<script type='text/javascript'>$('.play-".intval($_POST["id"])."').html('<font color=\"#0F680B\">������</font>');</script>";
		}
?>