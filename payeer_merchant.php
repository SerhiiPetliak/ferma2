...
if (isset($_POST["m_operation_id"]) && isset($_POST["m_sign"]))
{
   ...
   $ins_sum = $db->FetchRow();
   # первое пополнение +10% строчка $serebro = intval($ins_sum <= 0.01) ? ($serebro + ($serebro * 0.10) ) : $serebro;
   $serebro = intval($ins_sum <= 0.01) ? ($serebro + 500 + ($serebro * 0.00) ) : $serebro;
   $add_tree = ( $ik_payment_amount >= 499.99) ? 0 : 0;
   $lsb = time();
   # процент рефереру за пополнение реферала 10%
   ...
   # Зачисляем средства рефереру и животное - значение ноль не зачисляется
