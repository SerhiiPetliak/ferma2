...
<tr>
	...
	<td align="<?php echo (isset($this->_rootref['S_CONTENT_FLOW_END'])) ? $this->_rootref['S_CONTENT_FLOW_END'] : ''; ?>"><?php if ($this->_rootref['SEARCH_IN_RESULTS']) {  ?><span class="genmed"><?php echo ((isset($this->_rootref['L_SEARCH_IN_RESULTS'])) ? $this->_rootref['L_SEARCH_IN_RESULTS'] : ((isset($user->lang['SEARCH_IN_RESULTS'])) ? $user->lang['SEARCH_IN_RESULTS'] : '{ SEARCH_IN_RESULTS }')); ?>: </span><input class="post" type="text" name="add_keywords" value="" /> <input class="btnlite" type="submit" name="submit" value="<?php echo ((isset($this->_rootref['L_GO'])) ? $this->_rootref['L_GO'] : ((isset($user->lang['GO'])) ? $user->lang['GO'] : '{ GO }')); ?>" /><?php } ?></td>
</tr>
</table>
...
<?php if ($this->_rootref['S_SHOW_TOPICS']) {  ?>
	...
	<?php $_searchresults_count = (isset($this->_tpldata['searchresults'])) ? sizeof($this->_tpldata['searchresults']) : 0;if ($_searchresults_count) {for ($_searchresults_i = 0; $_searchresults_i < $_searchresults_count; ++$_searchresults_i){$_searchresults_val = &$this->_tpldata['searchresults'][$_searchresults_i]; ?><!-- begin mod: Prime Trash Bin --><?php if ($_searchresults_val['S_DELETED']) {  ?>

		<tr valign="middle" class="deleted_topic">
			...
			<td class="row1" colspan="5">
				...
				<?php } if ($_searchresults_val['S_TOPIC_GLOBAL']) {  ?>
					...
					<p class="gensmall"><?php echo ((isset($this->_rootref['L_GLOBAL'])) ? $this->_rootref['L_GLOBAL'] : ((isset($user->lang['GLOBAL'])) ? $user->lang['GLOBAL'] : '{ GLOBAL }')); ?> - <?php echo $_searchresults_val['DELETED_MSG']; ?></p>
				<?php } else { ?>

					<p class="gensmall"><?php echo ((isset($this->_rootref['L_IN'])) ? $this->_rootref['L_IN'] : ((isset($user->lang['IN'])) ? $user->lang['IN'] : '{ IN }')); ?> <a href="<?php echo $_searchresults_val['U_VIEW_FORUM']; ?>"><?php echo $_searchresults_val['FORUM_TITLE']; ?></a> - <?php echo $_searchresults_val['DELETED_MSG']; ?></p>
				<?php } ?>

			</td>
		</tr>
	<?php } else { ?><!-- end mod: Prime Trash Bin -->
		<tr valign="middle">
			<td class="row1" width="25" align="center"><?php echo $_searchresults_val['TOPIC_FOLDER_IMG']; ?></td>
			<td class="row1" width="25" align="center">
			<?php if ($_searchresults_val['TOPIC_ICON_IMG']) {  ?>
			...
			<td class="row1">
				...
				<?php echo $_searchresults_val['ATTACH_ICON_IMG']; ?> <a href="<?php echo $_searchresults_val['U_VIEW_TOPIC']; ?>" class="topictitle"><?php echo $_searchresults_val['TOPIC_TITLE']; ?></a>
				<?php if ($_searchresults_val['S_TOPIC_UNAPPROVED'] || $_searchresults_val['S_POSTS_UNAPPROVED']) {  ?>
		...
		<!-- begin mod: Prime Trash Bin --><?php } ?><!-- end mod: Prime Trash Bin --><?php }} else { ?>
...
<?php } else { ?>
	...
	<?php $_searchresults_count = (isset($this->_tpldata['searchresults'])) ? sizeof($this->_tpldata['searchresults']) : 0;if ($_searchresults_count) {for ($_searchresults_i = 0; $_searchresults_i < $_searchresults_count; ++$_searchresults_i){$_searchresults_val = &$this->_tpldata['searchresults'][$_searchresults_i]; ?>
		...
		<tr class="row2 <?php if ($_searchresults_val['S_DELETED']) {  ?> deleted_post<?php } ?>">
