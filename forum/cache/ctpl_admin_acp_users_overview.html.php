...
//-- end: Prime Trash Bin (Posts) -------------------------------------------//
	...
	<!-- begin mod: Prime Trash Bin --><!-- This is the form field used to specify a reason for deleting a user's posts. -->
	<div style="display: none;" id="delete_reason_area">
		<dl>
			<dt><label for="delete_reason"><?php echo ((isset($this->_rootref['L_PRIME_DELETE_POST_REASON'])) ? $this->_rootref['L_PRIME_DELETE_POST_REASON'] : ((isset($user->lang['PRIME_DELETE_POST_REASON'])) ? $user->lang['PRIME_DELETE_POST_REASON'] : '{ PRIME_DELETE_POST_REASON }')); ?>:</label></dt>
			<dd><input name="delete_reason" type="text" class="text medium" maxlength="100" id="delete_reason" /></dd>
		</dl>
		<?php if ($this->_rootref['S_CAN_DELETE_FOREVER']) {  ?>

		<dl>
			<dd><label for="delete_forever"><input type="checkbox" name="delete_forever" id="delete_forever" style="cursor:auto;" onchange="document.getElementById('delete_reason').disabled = this.checked;" /> <?php echo ((isset($this->_rootref['L_PRIME_DELETE_FOREVER'])) ? $this->_rootref['L_PRIME_DELETE_FOREVER'] : ((isset($user->lang['PRIME_DELETE_FOREVER'])) ? $user->lang['PRIME_DELETE_FOREVER'] : '{ PRIME_DELETE_FOREVER }')); ?></label></dd>
		</dl>
		<?php } ?>
