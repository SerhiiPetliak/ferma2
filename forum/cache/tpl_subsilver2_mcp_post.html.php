...
<?php } ?><!-- begin mod: Prime Trash Bin --><?php if ($this->_rootref['DELETED_REASON']) {  ?>

<tr>
	<td class="row1"><b class="gen" style="white-space:nowrap;"><?php echo ((isset($this->_rootref['L_PRIME_POST_DELETED_REASON'])) ? $this->_rootref['L_PRIME_POST_DELETED_REASON'] : ((isset($user->lang['PRIME_POST_DELETED_REASON'])) ? $user->lang['PRIME_POST_DELETED_REASON'] : '{ PRIME_POST_DELETED_REASON }')); ?>:</b></td>
	<td class="row2"><span class="postdetails"><?php echo (isset($this->_rootref['DELETED_REASON'])) ? $this->_rootref['DELETED_REASON'] : ''; ?></span></td>
</tr>
...
<tr>
	<td class="row1<?php if ($this->_rootref['S_DELETED']) {  ?> deleted_post<?php } ?>" colspan="2">
	...
	<?php if ($this->_rootref['S_TOPIC_REVIEW']) {  $this->_tpl_include('posting_topic_review.html'); } } else { if ($this->_rootref['S_CAN_LOCK_POST'] || $this->_rootref['S_CAN_DELETE_POST'] || $this->_rootref['S_CAN_CHGPOSTER']) {  ?>
		...
		<?php } if ($this->_rootref['S_CAN_LOCK_POST'] || $this->_rootref['S_CAN_DELETE_POST']) {  ?>
			...
			<tr>
				...
				<td class="row2"><form method="post" name="mcp" action="<?php echo (isset($this->_rootref['U_MCP_ACTION'])) ? $this->_rootref['U_MCP_ACTION'] : ''; ?>"><select name="action"><?php if ($this->_rootref['S_CAN_LOCK_POST']) {  if ($this->_rootref['S_POST_LOCKED']) {  ?><option value="unlock_post"><?php echo ((isset($this->_rootref['L_UNLOCK_POST'])) ? $this->_rootref['L_UNLOCK_POST'] : ((isset($user->lang['UNLOCK_POST'])) ? $user->lang['UNLOCK_POST'] : '{ UNLOCK_POST }')); ?> [<?php echo ((isset($this->_rootref['L_UNLOCK_POST_EXPLAIN'])) ? $this->_rootref['L_UNLOCK_POST_EXPLAIN'] : ((isset($user->lang['UNLOCK_POST_EXPLAIN'])) ? $user->lang['UNLOCK_POST_EXPLAIN'] : '{ UNLOCK_POST_EXPLAIN }')); ?>]</option><?php } else { ?><option value="lock_post"><?php echo ((isset($this->_rootref['L_LOCK_POST'])) ? $this->_rootref['L_LOCK_POST'] : ((isset($user->lang['LOCK_POST'])) ? $user->lang['LOCK_POST'] : '{ LOCK_POST }')); ?> [<?php echo ((isset($this->_rootref['L_LOCK_POST_EXPLAIN'])) ? $this->_rootref['L_LOCK_POST_EXPLAIN'] : ((isset($user->lang['LOCK_POST_EXPLAIN'])) ? $user->lang['LOCK_POST_EXPLAIN'] : '{ LOCK_POST_EXPLAIN }')); ?>]</option><?php } } if ($this->_rootref['S_CAN_DELETE_POST']) {  ?><option value="delete_post"><?php echo ((isset($this->_rootref['L_DELETE_POST'])) ? $this->_rootref['L_DELETE_POST'] : ((isset($user->lang['DELETE_POST'])) ? $user->lang['DELETE_POST'] : '{ DELETE_POST }')); ?></option><?php } if ($this->_rootref['S_CAN_UNDELETE_POST']) {  ?><option value="undelete_post"><?php echo ((isset($this->_rootref['L_PRIME_UNDELETE_POST'])) ? $this->_rootref['L_PRIME_UNDELETE_POST'] : ((isset($user->lang['PRIME_UNDELETE_POST'])) ? $user->lang['PRIME_UNDELETE_POST'] : '{ PRIME_UNDELETE_POST }')); ?></option><?php } ?></select> <input class="btnmain" type="submit" value="<?php echo ((isset($this->_rootref['L_SUBMIT'])) ? $this->_rootref['L_SUBMIT'] : ((isset($user->lang['SUBMIT'])) ? $user->lang['SUBMIT'] : '{ SUBMIT }')); ?>" />	<?php echo (isset($this->_rootref['S_FORM_TOKEN'])) ? $this->_rootref['S_FORM_TOKEN'] : ''; ?></form></td>
			</tr>
		<?php } ?>
