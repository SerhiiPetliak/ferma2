...
<?php } if ($this->_rootref['S_DISPLAY_POST_INFO'] || $this->_rootref['TOTAL_TOPICS']) {  ?>
	...
	<?php } if (! $this->_rootref['S_DISPLAY_ACTIVE'] && ( $this->_rootref['S_IS_POSTABLE'] || sizeof($this->_tpldata['topicrow']) )) {  ?>
		...
		<?php $_topicrow_count = (isset($this->_tpldata['topicrow'])) ? sizeof($this->_tpldata['topicrow']) : 0;if ($_topicrow_count) {for ($_topicrow_i = 0; $_topicrow_i < $_topicrow_count; ++$_topicrow_i){$_topicrow_val = &$this->_tpldata['topicrow'][$_topicrow_i]; ?><!-- begin mod: Prime Trash Bin --><?php if ($_topicrow_val['S_DELETED']) {  ?>
			...
			<tr class="deleted_topic">
				...
				<td class="row1" colspan="5">
					...
					<?php echo $_topicrow_val['ATTACH_ICON_IMG']; ?> <?php if ($_topicrow_val['S_TOPIC_MOVED']) {  ?><b><?php echo $_topicrow_val['TOPIC_TYPE']; ?></b> <?php } ?><a title="<?php echo ((isset($this->_rootref['L_POSTED'])) ? $this->_rootref['L_POSTED'] : ((isset($user->lang['POSTED'])) ? $user->lang['POSTED'] : '{ POSTED }')); ?>: <?php echo $_topicrow_val['FIRST_POST_TIME']; ?>" href="<?php echo $_topicrow_val['U_VIEW_TOPIC']; ?>" class="topictitle"><?php echo $_topicrow_val['TOPIC_TITLE']; ?></a><br /><?php echo $_topicrow_val['DELETED_MSG']; ?>
		...
		<?php } else { ?><!-- end mod: Prime Trash Bin --><?php if ($_topicrow_val['S_TOPIC_TYPE_SWITCH'] == (1)) {  ?>
...
<!-- begin mod: Prime Trash Bin --><?php } ?><!-- end mod: Prime Trash Bin --><?php }} else { if ($this->_rootref['S_IS_POSTABLE']) {  ?>
